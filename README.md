# Betaguard - Motion's event detecion & real time notification / 
...
Assicurarsi che la folder dei media di Motion sia "/etc/lib/motion/", atrimenti modificare il path in infono allo script "alert.sh".

ISTRUZIONI:

1- Rinominare "alert2.sh" ad "alert.sh"  
2- Modificare i path in cima allo script betaguard.sh ed ad alert.sh(inserendo anche i dati del tuo mailer fake per l'invio della email).
...
Make sure the Motion media folder is "/ etc / lib / motion /", otherwise change the path to below the "alert.sh" script.
...
Oh, right thanks. Yes, i write scripts with my Bs.
